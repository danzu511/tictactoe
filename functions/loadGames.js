...
export function createGameFromColors(game, scene, baseColor, skipColorParse) {
  const dim = Math.sqrt(game.turns[0].length);
  const newGame = [];
  // Iterate each turn
  for (let j = 0; j < game.turnCount; j++) {
    const newTurn = new Grid(dim, dim, scene, baseColor);
    ...
    for (let o = 0; o < newTurn.cells.length; o++) {
      if (skipColorParse) {
        ...
        const newColorStr = `0x${game.turns[j][o].toString(16)}`;
        const newColorInt = parseInt(newColorStr, 16);
